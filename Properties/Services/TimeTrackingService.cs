...
public class TimeTrackingService
{
    ...
    /// <summary>Blijft aanwezig zodat pagina's 'm gerust kunnen aanroepen; doet bij lokale
    /// opslag niets extra's (in tegenstelling tot de gedeelde Firestore-versie).</summary>
    public Task RefreshProjectsAsync() => Task.CompletedTask;
    ...
    /// <summary>Verwijdert een opdracht definitief (in tegenstelling tot archiveren, wat omkeerbaar is).</summary>
    ...
    /// <summary>Verwijdert alle segmenten (reistijd/werk/pauze) van één specifieke dag definitief.</summary>
    public async Task DeleteDaySegmentsAsync(DateOnly date)
    {
        _segments.RemoveAll(s => DateOnly.FromDateTime(s.Start) == date);
        await _storage.SetListAsync(SegmentsKey, _segments);
        NotifyChange();
    }
